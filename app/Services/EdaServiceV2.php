...
class EdaServiceV2
{
    ...
    public function getEngineRpmLimitByAfml($acReg, $dateStart, $dateEnd, $rpmLimit, $withEmpty = false)
    {
        $aircraft = DB::table('aircraft')->select('id')->where('reg_code', $acReg)->first();
        ...
        if ($afmlRecords->isEmpty()) return ['flights' => []];

        $results = [];

        foreach ($afmlRecords as $afml) {
            $flights = DB::table('afml_detail as ad')
                ->select(['ad.id', 'ad.to as takeoff_time', 'it_from.code as from_code', 'it_from.icao_code as from_icao', 'it_to.code as to_code'])
                ->leftJoin('iata as it_from', 'it_from.id', '=', 'ad.iata_id_from')
                ->leftJoin('iata as it_to', 'it_to.id', '=', 'ad.iata_id_to')
                ->where('ad.afml_id', $afml->id)
                ->get();
            ...
            foreach ($flights as $flight) {
                ...
                if ($csvFile) {
                    $flightDate = $this->getDateFromCsv($csvFile);
                    $rpmData = $this->calculateMetricForCsv($csvFile, $rpmLimit, 'E1 NP');
                    
                    $crewInfo = DB::table('afml as a')
                        ->select(['tu_1.full_name as captain', 'tu_2.full_name as copilot', 'tu_3.full_name as engineer'])
                        ->leftJoin('tb_user as tu_1', 'tu_1.id', '=', 'a.captain_user_id')
                        ->leftJoin('tb_user as tu_2', 'tu_2.id', '=', 'a.copilot_user_id')
                        ->leftJoin('tb_user as tu_3', 'tu_3.id', '=', 'a.engineer_user_id')
                        ->where('a.id', $afml->id)
                    ...
                    if ($rpmData['total_overlimit_events'] > 0 || $withEmpty) {
                        $results[] = [
                            'afml_id' => $afml->id,
                            'page_no' => $afml->page_no,
                            'date' => $flightDate ?? $afml->date,
                            ...
                            'takeoff_time' => $flight->takeoff_time,
                            'csv_file' => basename($csvFile),
                            'overlimit_events' => $rpmData['total_overlimit_events'],
                            'overlimit_duration' => $rpmData['total_overlimit_duration'],
                            'captain' => $crewInfo->captain ?? '',
                            'copilot' => $crewInfo->copilot ?? '',
                            'engineer' => $crewInfo->engineer ?? '',
                            'details' => $rpmData['details']
                        ];
                    }
                }
            }
    ...
    public function getEngineGasLimitByAfml($acReg, $dateStart, $dateEnd, $percentageLimit, $withEmpty = false)
    {
        $aircraft = DB::table('aircraft')->select('id')->where('reg_code', $acReg)->first();
        if (!$aircraft) return ['error' => 'Aircraft not found'];
        ...
        foreach ($afmlRecords as $afml) {
            ...
            foreach ($flights as $flight) {
                ...
                if ($csvFile) {
                    $flightDate = $this->getDateFromCsv($csvFile);
                    $gasData = $this->calculateMetricForCsv($csvFile, $percentageLimit, 'E1 NG');
                    ...
                    if ($gasData['total_overlimit_events'] > 0 || $withEmpty) {
                        $results[] = [
                            'afml_id' => $afml->id,
                            'page_no' => $afml->page_no,
                            'date' => $flightDate ?? $afml->date,
                            ...
                            'takeoff_time' => $flight->takeoff_time,
                            'csv_file' => basename($csvFile),
                            'overlimit_events' => $gasData['total_overlimit_events'],
                            'overlimit_duration' => $gasData['total_overlimit_duration'],
                            'captain' => $crewInfo->captain ?? '',
                            'copilot' => $crewInfo->copilot ?? '',
                            'engineer' => $crewInfo->engineer ?? '',
                            'details' => $gasData['details']
                        ];
                    }
                }
            }
    ...
    private function calculateMetricForCsv($filePath, $limit, $columnName)
    {
        $handle = fopen($filePath, 'r');
        if (!$handle) {
            return ['total_overlimit_events' => 0, 'total_overlimit_duration' => '00:00:00', 'details' => []];
        ...
        foreach ($headers as $index => $header) {
            $header = trim($header);
            if ($header === $columnName) $metricColumnIndex = $index;
            if ($header === 'Lcl Date') $dateColumnIndex = $index;
            if ($header === 'Lcl Time') $timeColumnIndex = $index;
        }
        ...
        while (($line = fgets($handle)) !== false) {
            $data = str_getcsv($line);
            if (count($data) <= $metricColumnIndex) continue;
            
            $value = floatval(trim($data[$metricColumnIndex]));
            $date = $dateColumnIndex >= 0 && isset($data[$dateColumnIndex]) ? trim($data[$dateColumnIndex]) : '';
            $time = $timeColumnIndex >= 0 && isset($data[$timeColumnIndex]) ? trim($data[$timeColumnIndex]) : '';
            ...
            if ($value > $limit && !$inOverlimit) {
                ...
                $overlimitStartTime = $currentTime;
                $events++;
            } elseif ($value <= $limit && $inOverlimit) {
                $inOverlimit = false;
                if ($overlimitStartTime) {
                    $duration = $currentTime->diffInSeconds($overlimitStartTime);
                    $totalSeconds += $duration;

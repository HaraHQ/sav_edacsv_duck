...
classified output with all seven result columns:
    ...
    FLIGHT_PHASE     Phase label (CRUISE, APPROACH, FINAL APPROACH, TAKEOFF, LANDING, ...)
                     ...
                     - FINAL APPROACH (final approach ≤ 500ft AGL)
...
class BatchFlightProcessor:
    ...
    def process_single_file(self, input_file: Path,
        ...
        try:
            ...
            # ── Classify ───────────────────────────────────────────────────────
            # Use strict Regex to capture only the 6 digits after "log_"
            m_date = re.search(r'log_(\d{6})_', input_file.name, re.IGNORECASE)
            flight_date = str(m_date.group(1)) if m_date else ""
            
            m_icao = re.search(r'_([A-Z0-9]{4})\.csv$', input_file.name, re.IGNORECASE)
            dep_icao = m_icao.group(1).upper() if m_icao else ""
            ...
            # ── Extract Flight Times (Pure Python approach) ───────────────────
            to_idx = -1
            ldg_idx = -1
            
            if 'FLIGHT_PHASE' in df_work.columns:
                phase_col = df_work['FLIGHT_PHASE']
                if isinstance(phase_col, pd.DataFrame):
                    phase_col = phase_col.iloc[:, 0]
                
                phases_list = phase_col.astype(str).tolist()
                
                try:
                    to_idx = phases_list.index('TAKEOFF')
                except ValueError:
                    pass
                    
                try:
                    ldg_idx = len(phases_list) - 1 - phases_list[::-1].index('LANDING')
                except ValueError:
                    pass
            ...
            if to_idx >= 0 and ldg_idx >= 0:
                if 'Lcl Time' in df_dedup.columns:
                    time_col = df_dedup['Lcl Time']
                    if isinstance(time_col, pd.DataFrame):
                        time_col = time_col.iloc[:, 0]
                    
                    time_list = time_col.astype(str).tolist()
                    
                    raw_to_time = time_list[to_idx] if to_idx < len(time_list) else ""
                    raw_ldg_time = time_list[ldg_idx] if ldg_idx < len(time_list) else ""
                    
                    t_start_str = ":".join(raw_to_time.split(':')[:2])
                    t_end_str = ":".join(raw_ldg_time.split(':')[:2])
                
                # Convert seconds difference to HH:MM format
    ...
    def _handle_already_classified(self, input_file: Path,
        ...
        try:
            ...
            # Aggregate stats for skipped files
            for phase, count in phase_counts.items():
                self.summary_stats['phase_distribution'][phase] = (
                    self.summary_stats['phase_distribution'].get(phase, 0) + count)
            for lbl, cnt in event_counts.items():
    ...
    def _write_summary_report(self) -> None:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            for entry in self.processing_log:
                ...
                f.write(f'Takeoff  : {entry.get("takeoff_time", "")}\n')
                f.write(f'Landing  : {entry.get("landing_time", "")}\n')
                f.write(f'Duration : {entry.get("flight_duration", "00:00")}\n')
                f.write(f'Rows     : {rows:,}\n')
